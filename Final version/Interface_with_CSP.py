...
from Problem_initialization import CSP
...
Hospitals_file = os.path.join(
    os.path.dirname(__file__), "data", "Hospitals_Clinics.csv"
)
...
img = img.resize(
    (int(master.winfo_screenwidth()), int(master.winfo_screenheight() * 0.995))
)
photo = ImageTk.PhotoImage(img)
canvas = Canvas(
    master, width=master.winfo_width() + 600, height=master.winfo_height() + 300
)
...
w.config(
    font=("Arial", 14), bg="#B9D9EB", fg="black", width=20
...
coordinates = Entry(
    master, font=("Arial", 12), fg="black", width=30, borderwidth=2, justify="center"
)
...
try:
    ...
    # -----------finding the nearest hospital using CSP---------------
    # open the file of the hospitals
    with open(Hospitals_file, "r") as csv_file:
        ...
        for hospital in csv_reader:
            ...
            ID, _, facility, coordinate, services, Capacity = hospital

            X, Y = coordinate.split(",")
            distance = Harvensine.Distance_Haversine(
                initial_coord, (float(X), float(Y))
            )
    ...
    constraints = (selected_service, float(dist))

    csp_problem = CSP.CSP(all_hospitals, constraints)
    hospital = csp_problem.best_hospital()

    # find the closest inter to the Hospital
    goal_node_id = userLocation.find_closest_node(hospital["coordinates"], graph)
    ...
    hospital_location = tuple(map(float, hospital["coordinates"]))
    folium.Marker(
